 ...
 *
 * Wichtig:
 * - Diese Datei exportiert wirklich `Planner`.
 * - Brain.js kann also wieder korrekt importieren:
 ...
 * - EMPTY_LOAD
 * - COLLECT_TRASH
 * - AVOID mit Safety-Abstand
 * - PATROL_AREA für Ortsgedächtnis
 * - RETURN_HOME
 * - STANDBY
 * - EXPLORE
 */
export class Planner {
    ...
    plan(goal, worldModel, robot, observation, emotions, semantic = null, spatial = null) {
        ...
        switch (goal.type) {
            ...
            case GOAL.PATROL_AREA:
                action = this._planPatrolArea(goal, worldModel, robot);
                break;
    ...
    _planCharge(worldModel, robot) {
        ...
        return this._moveToward(
            robot,
            worldModel.home,
            worldModel,
            "Zur Ladestation bewegen"
        );
    }
    ...
    _planReturnHome(worldModel, robot) {
        ...
        return this._moveToward(
            ...
            worldModel.home,
            worldModel,
            "Mission abgeschlossen, Rückkehr zur Basis"
        );
    }
    ...
    _planPatrolArea(goal, worldModel, robot) {
        const target = goal.target;

        if (!target) {
            return {
                type: ACTION.SCAN,
                reason: "Keine Patrouillenregion bekannt, scanne Umgebung."
            };
        }
        ...
        if (manhattan(robot, target) <= 1) {
            return {
                type: ACTION.SCAN,
                reason:
                    `Region ${goal.regionId} erreicht. Scanne erneut, weil diese Region ` +
                    "historisch interessant war."
            };
        }
        ...
        return this._moveToward(
            ...
            target,
            worldModel,
            `Region ${goal.regionId} erneut prüfen`
        );
    }
    ...
    _planAvoid(goal, worldModel, robot, semantic = null) {
        ...
        const relevantSemanticHazards = semanticHazards.filter(
            item => item.distance < safeDistance + 1
        );

        const fallbackHazards = worldModel
            .getKnownHazards(robot.position, safeDistance + 1)
            .map(entity => ({
                id: entity.id,
                label: entity.label,
                entity,
        ...
        const hazards =
            relevantSemanticHazards.length > 0
